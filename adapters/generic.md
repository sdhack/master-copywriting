...
> **Target**: Any markdown-capable agent
> **Default Mode**: TEXT_ONLY
> **Purpose**: Baseline compatibility. Works with any agent that can read markdown.

---

## Installation

1. Copy the entire `master-copywriting-v4` folder to a location the agent can access
2. The agent reads `SKILL.md` as the entry point
3. Reference files are loaded as markdown when needed
...
| FILE_READ | ❌ Not available (may work if agent has file access) |
| FILE_SEARCH | ❌ Not available |
| CODE_EXECUTION | ❌ Not available |
| CALCULATOR | ❌ Manual calculation only |
...
- Runs entirely from `SKILL.md` canonical core
- References are loaded by reading the markdown files if file access is available
- Without file access: relies on SKILL.md core principles + agent's in-context understanding
- All hard gates enforced as checklist-based QA
- If WEB_SEARCH becomes available: it can be used for **Product Source Retrieval** (establish P1 Product Facts for the current SKU from verified official pages) AND **External Intelligence** AND **current fact verification**; Product Source Retrieval has priority when product facts are insufficient for the current purpose
- No fabricated web results, trends, or competitor data

## Context Loading

- Load `SKILL.md` first
- If user task requires specific references, read those files
...
- Skill directory is readable
- Reference paths are relative to `SKILL.md`
- No write access required for standard operation

## Canonical Rule Compliance

This adapter **does not modify any canonical rules**. It only maps (or removes) capabilities.
