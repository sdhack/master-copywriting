# Adapter: Limited Agent

> **Target**: Mobile agents, chat-only agents, limited UI environments
> **Default Mode**: TEXT_ONLY (may upgrade to WEB_ONLY if search available)
> **Purpose**: Minimum viable skill operation on constrained platforms

---

## Installation

1. Copy SKILL.md content as system prompt / instructions
2. Reference files may not be accessible — key rules inlined in SKILL.md
3. No file write, no code execution, no MCP

## Capability Mapping

| Abstract Capability | Implementation |
|---|---|
| WEB_SEARCH | ❌ / ⚠️ (depends on agent) |
| FILE_READ | ❌ Not available |
| FILE_SEARCH | ❌ Not available |
| CODE_EXECUTION | ❌ Not available |
| CALCULATOR | ❌ Manual only |
| FUNCTION_CALLING | ❌ Not available |
| MCP | ❌ Not available |
| MEMORY | ❌ / ⚠️ (conversation-level only) |
| STRUCTURED_OUTPUT | ❌ Not available |

## Behavior

### What works
- Core routing (text-based)
- Hard gate checklist (manual)
- Platform core principles
- Purpose differentiation
- Fact boundary awareness
- IP identity integrity
- Final output sanitization

### What degrades
- **External intelligence**: Disabled by default. No real-time research, no trend-based angles. Must use only user-provided facts.
- **Product Source Retrieval**: If the limited agent gains WEB_SEARCH, it can retrieve verified official product pages to build a Product Ledger; without WEB_SEARCH it must rely on user-provided facts (PRODUCT_FACT_LIMITED_MODE).
- **Reference depth**: Only what's in SKILL.md + agent's general knowledge. No deep reference diving.
- **Numeric validation**: Manual, conservative. Reduced calculation claims.
- **Schema validation**: Not available.
- **Regression testing**: Not available.
- **Content history**: Only within current conversation.

### What must never happen
- Fabricating web search results
- Claiming to have read reference files that weren't loaded
- Simulating code execution or validation
- Fabricating product facts to fill gaps
- Claiming cross-session memory

## Context Loading

- Only SKILL.md canonical core
- No reference file loading (no file access)
- Core principles must be self-contained in SKILL.md

## Output Limitations

- No structured JSON
- No automated QA reports
- Manual gate checking only
- May need to be more conservative with claims

## Upgrade Path

If the limited agent gains web search capability → upgrade to WEB_ONLY mode.
If it gains file access → upgrade to GROUNDED mode.

## Canonical Rule Compliance

This adapter **does not modify any canonical rules**. It runs the full canonical core but with reduced capability set and appropriate graceful degradation.
