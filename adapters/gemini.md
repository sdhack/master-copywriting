...
> **Target**: Google Gemini agents / Gemini API
> **Default Mode**: FULL (when all tools available)
> **Purpose**: Full capability utilization on Google Gemini platform

---

## Installation

1. Package as Gemini system instruction + ground resources
2. Entry point: `SKILL.md` content in system instructions
3. Reference files via grounding / Google Drive
4. Code execution for validation
5. Google Search for external intelligence

## Capability Mapping

| Abstract Capability | Gemini Implementation |
|---|---|
| WEB_SEARCH | Google Search grounding / Search tool |
| FILE_READ | Google Drive file access / File API |
| FILE_SEARCH | Google Drive search / grounding search |
| CODE_EXECUTION | Code execution (Python/JavaScript) |
| CALCULATOR | Code execution (math) |
| FUNCTION_CALLING | Function calling / Tools |
| MCP | Extensions / Actions framework |
| MEMORY | Context caching / long context window |
| STRUCTURED_OUTPUT | Structured outputs / JSON mode / schema constraints |

## Runtime Negotiation

1. Check available tools in the current environment
2. If Google Search + Drive access + code execution → FULL mode
3. If Drive but no Search → GROUNDED mode
4. If Search but no Drive → WEB_ONLY mode
5. If basic API only → TEXT_ONLY mode

## Tool Usage Notes

### Google Search
- **Product Source Retrieval** via Google Search grounding: establish P1 Product Facts for the current SKU from verified official pages (build a Product Ledger)
- **External Intelligence**: user question / trend discovery / competitor context
...
- Always respect External Claim Admission Gate
- Categorize web signals properly (QUESTION / TENSION / TREND / etc.)
- Follow Product Acquisition rules from `execution/product-acquisition.md`

### Google Drive
- References stored in Drive folder
- Load via file API
- Product/IP facts from Drive documents
...
### Structured Output
- Gemini supports structured output natively
- Use schemas from `schemas/` for validation
- Route instances, research briefs, content fingerprints

## Context Loading Behavior

Gemini has very large context windows. Can load more references per session, but progressive disclosure still recommended for focus.

**Recommended pattern:**
1. SKILL.md in system instructions (always)
2. Load core execution rules
3. Route the task
4. Load platform/purpose-specific references
5. Generate and validate

## Platform-Specific Fields

- Gemini-specific fields (safety settings, temperature, etc.) are adapter-level
- Canonical core is platform-agnostic
- Grounding sources, model config, and tool definitions live in adapter config

## Canonical Rule Compliance

This adapter **does not modify any canonical rules**. It only maps abstract capabilities to Gemini's specific tools and APIs.
