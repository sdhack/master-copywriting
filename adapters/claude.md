...
> **Target**: Claude (Anthropic) agents with tool use
> **Default Mode**: FULL (when all tools available)
> **Purpose**: Full capability utilization on Claude platform

---

## Installation

1. Install as a Claude Skill using the standard skill format
2. Entry point: `SKILL.md`
3. Reference files loaded via file reading tool
4. External intelligence via web search tool
5. Validation via code interpreter / Claude Code

## Capability Mapping

| Abstract Capability | Claude Implementation |
|---|---|
| WEB_SEARCH | `web_search` / `web_fetch` tools (Claude's built-in web access) |
| FILE_READ | File reading tool / directory listing |
| FILE_SEARCH | `grep` via code execution, or built-in search |
| CODE_EXECUTION | Claude Code / bash tool (Python, Node.js) |
| CALCULATOR | Code execution (math) |
| FUNCTION_CALLING | Native tool use (MCP + built-in tools) |
| MCP | MCP server connections |
| MEMORY | Claude memory feature (if enabled) |
| STRUCTURED_OUTPUT | JSON mode / tool-calling schemas |

## Runtime Negotiation

1. Check available tools on activation
2. If web_search + file_read + code_execution + MCP → FULL mode
3. If file_read but no web → GROUNDED mode
4. If web but no file access → WEB_ONLY mode
5. If none → TEXT_ONLY mode (fallback to generic)

## Tool Usage Notes

### Web Research
- Use `web_search` for **Product Source Retrieval** (establish P1 Product Facts for the current SKU from verified official pages) AND **External Intelligence** (question discovery, signal collection, trend/competitor context) AND **current fact verification**
- Use `web_fetch` for deep dives on authoritative sources
...
- Follow External Claim Admission Gate from `execution/expression-authority.md`
- Follow Product Acquisition rules from `execution/product-acquisition.md`

### File Access
- References loaded progressively via file reading
- Use relative paths from SKILL.md location
- Product/IP facts can be loaded from local files

### Code Execution
- Run validation scripts from `scripts/` directory
- Run regression test suite
- Schema validation for structured inputs/outputs

### MCP
- Product facts can come from MCP product databases
- IP profiles can come from MCP knowledge bases
- Content history for repetition checking

## Context Loading Behavior

Claude has large context windows. Progressive disclosure still recommended but can load more references per session.

**Recommended loading pattern:**
1. SKILL.md (always)
2. execution/execution-reliability.md (always — hard gates)
3. Mode-specific references (after routing)
4. Additional references (as needed during writing)

## Platform-Specific Fields

- Use of `allowed-tools` or similar fields is optional enhancement only
- Canonical core works without any platform-specific fields
- If platform-specific fields are used, they're for optimization, not core behavior

## Canonical Rule Compliance

This adapter **does not modify any canonical rules**. It only maps abstract capabilities to Claude's specific tools.
