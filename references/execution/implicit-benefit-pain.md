...
> **版本**：v4.11.0 更新（I5 分级 / PARTIAL_PAIN / Back-Translation 可恢复）
...
> **解决问题**：Skill 对食品、营养品、美妆等品类仍存在——发现功效风险 → 删除功效 → 文案失去痛点 → 最后只能介绍产品参数。本轮显著放宽：用户利益暗示 / 场景暗示 / 生活结果暗示 / 痛点翻译 / 情绪价值 / 使用价值。
> **最高原则**：# Loosen implication around value, not around unauthorized efficacy.
> 不是：把"放宽暗示"理解成通过换词、暗语、剧情、表情、谐音继续表达一个本身没有权限的医疗/保健/生理功效。
> **v4.11.0 关键变化**：I5 从一刀切禁止拆分为 I5-A / I5-B / I5-C 三级；Pain Distance 增加 PARTIAL_PAIN；Semantic Destination Test 从"硬拒"改为"可恢复"（越线 → CONVERSION_RECOVERY）。
...
**谨慎开放。** 状态差 / 疲劳 / 皮肤焦虑 / 年龄焦虑 / 熬夜 / 饮食不规律等，允许作为 **Audience Context** 出现。

允许条件（全部满足）：

1. 只作为受众状态描述，不绑定成产品治疗目标；
2. 必须落地到产品实际解决的摩擦（携带 / 搭配 / 选择 / 执行 / 计量 / 口感）；
3. 主语义不能变成"产品在改生理"——Semantic Back-Translation 后，消费者理解的主效果必须是合法价值域（省事 / 更简单 / 更容易坚持 / 选择更少）。
...
> "天天外卖身体亏空，吃这个补回来。"（把产品绑成生理修复）

#### I5-C — Acceptable Strong Implication（可接受强暗示）

**开放（非敏感品类）。** 在普通食品 / 日用品 / 非宣称功效化妆品下，允许更强的情绪、身份、生活方式暗示——只要 Semantic Back-Translation 后主语义仍落在合法价值域。
...
边界：主语义必须落在 Convenience / Routine Simplicity / Choice / Identity / Lifestyle / Sensory 等合法价值域。若 Back-Translation 后消费者理解的主效果变成"产品改善某种身体状态"→ 降级为 I5-B 处理（落地到摩擦）或直接 FAIL。

**COMMERCIAL_INTENSITY 联动**：I5-B 在 STANDARD / AGGRESSIVE 下允许；I5-C 仅在 AGGRESSIVE 且非敏感品类下开放，CONSERVATIVE 下 I5-B/C 均收紧为"只做受众上下文、不强化"。
...
**v4.11.0 起：失败不是终点，而是 CONVERSION_RECOVERY 的入口。**

检测到语义终点越线后：

1. **保留情绪强度和场景冲击力**——不删除整段利益表达；
2. **只替换越线的那一跳**——把"产品改善身体"那一跳换成"产品把这件事变简单"的合法价值跳；
3. 禁止直接删成参数列表（那等于放弃销售）。

示例：

- 越线："下午三点不垮了。"（主语义 = 产品改善疲劳）
- 恢复："下午不用再靠第三杯咖啡硬撑，是很多人的真实状态；这种按天分装，至少让每天这件事不用再想。"（主语义 = 执行更简单 / 省事）
...
（原 INFERRED_PHYSIOLOGICAL_PAIN 归入此类：需要额外身体效果推导的痛点，只能作为受众上下文，不得作为主卖点。）

写法：作为 Audience Context 引入 → 立即落地到产品真实解决的摩擦 → 主语义停在合法价值域。

**COMMERCIAL_INTENSITY 联动**：DIRECT_PAIN 在任何强度下都大胆写；PARTIAL_PAIN 在 STANDARD / AGGRESSIVE 下强表达、CONSERVATIVE 下也禁止免责声明（边界体现在不夸大）；CONTEXT_PAIN 在任何强度下都只能作为上下文，不得当主卖点。
